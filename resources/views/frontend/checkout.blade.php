        ...
        <div class="row billing">
                                ...
                                <div class="card-body">
                                    <div id="shippingCollapse" class="collapse show" aria-labelledby="shippingHeading" data-parent="#shippingAccordion">
                                            ...
                                            <div class="row address-form">
                                                ...
                                                <div class="col-lg-6 form-group mb-3">
                                                    <label for="">Postal Code</label>
                                                    <input class="form-control postalCode_fill_delivery" autocomplete="off" type="text"   value="{{session()->has('postalcode') && strlen(session('postalcode')) >= 5 ? session('postalcode') :''}}"
                                                ...
                                                <div class="col-lg-6 form-group mb-3">
                                                    ...
                                                    <select name="s_province" readonly class="form-control  p-2" id="s_province">
                                                       @foreach ($province as $province_items)
                                                            <option value="{{ $province_items->name }}" @if(session('province') == $province_items->name) selected @elseif("Ontario" == $province_items->name) selected @else disabled @endif>
                                                                {{ $province_items->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                        ...
                        @foreach ($items as $listing)
                            @if($listing->product_variation)
                                ...
                                if($listing->has_special_price == 1){
                                    ...
                                    {
                                        \App\Models\Item::where('id',$listing->id)->update(['has_special_price' => 0,'price_amount' => $listing->product_variation->price,'special_price_from' => null,'special_price_to' => null]);
                                    }
                                        $listing->refresh();
                                }
                                else{
                                ...
                                <div class="products-checkout col-12 mb-3 products-cart">
                                    <div class="p-c">
                                        ...
                                        <div class="i-content" style="padding-left: 5px">
                                            ...
                                            <div class="d-flex">
                                                <p class="text-theme fw-bold">{{ getPrice($listing->price_amount) }}</p>
                                                @if($checkSpecialPrice)
                                                    ...
                                                    @endphp
                                                   <small class="text-body-tertiary me-1"> (<strike>{{getPrice($listing->actual_price)}}</strike>) <span class="text-danger">{{$discountType}}</span></small>
                                                @endif
                                            </div>
            ...
            <div class="col-md-8 mt-2">
                ...
                <div class="text-start mb-3 mt-3 text-center">
                    ...
                    <button class="g-recaptcha btn  btn-sm primary-btn mt-1" data-sitekey="{{ config('services.recaptcha_v3.siteKey') }}"
                                    data-callback="onSubmit"
                                    data-action="submitORder"  form="place__order" type="submit" >COMPLETE PURCHASE</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function onSubmit(token) {
        ...
        $("#place__order").trigger("submit");
         ...
         setTimeout(function(){    grecaptcha.reset(); }, 10000);
    }
</script>
...
<script>  
    ...
    $('body').on('change','#same_billing',function(e){
        ...
        @if(auth()->check())
            ...
            $('#s_firstname').val(databilling.data('firstname'));
            $('#s_lastname').val(databilling.data('lastname'));
