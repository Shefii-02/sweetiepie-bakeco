            ...
            if( ($pay->getResponseCode() < 50 && strlen($refNum) > 5 && strlen($txnNum) > 5 && is_numeric($resCod)) || env('APP_URL') == 'https://www.sweetiepiebakeco.indigitalapi.com')
            {
                ...
                //order Confirmation mail
                    ...
                    //order notification
                        ...
                        //     if(env('APP_URL') != 'https://www.sweetiepiebakeco.indigitalapi.com'){
                        //         if($store){
                        //             if($basket->order_type == 'delivery') {
                        ...
                        //     }
                        // try {
                        //     Mail::to($all_ordersSend)->bcc($cc_mailId)->send(new OrderNotification($order_details)); // ->cc()->cc($cc_mailId)->bcc(env('MAIL_TO_COPY'))
                        
                        //     $order->store_email_send  = 1;
                        //     $order->save();
                        // }
                        // catch(\Exception $e) {
                        // }
